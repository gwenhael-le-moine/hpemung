...
static inline void update_fields( void )
{
    ...
    cpu.fl_idx_hi[ 1 ] = cpu.fl_idx_hi[ 9 ] = cpu.p + 1;
}
...
static inline int nib_to_signed( byte* nib, int len )
{
    int x;
...
static inline address rstk_pop( void )
{
...
static inline void rstk_push( address adr )
{
    cpu.rstk_ptr = ( cpu.rstk_ptr - 1 ) & 7;
    cpu.rstk[ cpu.rstk_ptr ] = adr & 0xFFFFF;
}

static inline void goyes( byte* opc, int offset )
{
    if ( cpu.carry ) {
...
static inline void reg_bit( byte* reg, int bit, int value )
{
    if ( value ) {
        reg[ bit >> 2 ] |= 1 << ( bit & 3 );
    } else {
...
static inline void reg_cpy( byte* dest, byte* src, int len ) { memcpy( dest, src, len ); }

static inline void reg_ex( byte* reg1, byte* reg2, int len )
{
    byte tmp[ 16 ];
...
static inline void comp_bit_zero( byte* reg, int bit ) { cpu.carry = ( reg[ bit >> 2 ] & ( 1 << ( bit & 3 ) ) ) ? false : true; }

static inline void comp_zero( byte* reg, int len )
{
    while ( len-- ) {
        if ( *reg++ ) {
            cpu.carry = false;
...
static inline void comp_eq( byte* reg1, byte* reg2, int len )
{
    while ( len-- ) {
        if ( *reg1++ != *reg2++ ) {
            cpu.carry = false;
...
static inline void comp_gt( byte* reg1, byte* reg2, int len )
{
...
static inline void alu_add( byte* dest, byte* src, int len )
{
    byte c = 0;
...
static inline void alu_sub( byte* dest, byte* src, int len )
{
    byte c = 0;
    byte base = cpu.hexmode ? 16 : 10;
...
static inline void alu_sub2( byte* dest, byte* src, int len )
{
    byte c = 0;
    byte base = cpu.hexmode ? 16 : 10;
...
static inline void alu_add_con( byte* reg, byte con, int i, int len )
{
    reg[ i ] += con;
    while ( len-- ) {
        reg[ i ]++;
...
static inline void alu_sub_con( byte* reg, byte con, int i, int len )
{
    reg[ i ] -= con;
    while ( len-- ) {
        reg[ i ]--;
...
static inline void alu_inc( byte* reg, int len )
{
    if ( !cpu.hexmode ) {
        byte c = 1;
        while ( len-- ) {
...
static inline void alu_dec( byte* reg, int len )
{
    byte base = cpu.hexmode ? 16 : 10;
...
static inline void alu_neg( byte* reg, int len )
{
    byte base = cpu.hexmode ? 16 : 10;
...
static inline void alu_not( byte* reg, int len )
{
...
static inline void alu_and( byte* dest, byte* src, int len )
{
    while ( len-- ) {
        *dest++ &= *src++;
    }
...
static inline void alu_or( byte* dest, byte* src, int len )
{
    while ( len-- ) {
        *dest++ |= *src++;
    }
...
static inline void alu_sl( byte* reg, int len )
{
    while ( --len ) {
        reg[ len ] = reg[ len - 1 ];
    }
...
static inline void alu_slc( byte* reg, int len )
{
    byte tmp = reg[ len - 1 ];
...
static inline void alu_sr( byte* reg, int len )
{
    if ( reg[ 0 ] )
        cpu.hst |= HST_SB;
...
static inline void alu_src( byte* reg, int len )
{
    byte tmp = reg[ 0 ];
...
static inline void alu_srb( byte* reg, int len )
{
    if ( *reg & 1 )
        cpu.hst |= HST_SB;
...
{
    int len = ( opc[ 2 ] == 0xF ) ? 5 : cpu.fl_idx_hi[ opc[ 2 ] ];
    if ( opc[ 3 ] & 8 )
...
{
    if ( !( opc[ 2 ] & 2 ) ) {
        bus_write( cpu.reg[ ( opc[ 2 ] & 4 ) >> 1 ] + cpu.fs_idx_lo[ opc[ 3 ] ], cpu.d[ opc[ 2 ] & 1 ], cpu.fl_idx_hi[ opc[ 3 ] ] );
    } else {
        bus_read( cpu.reg[ ( opc[ 2 ] & 4 ) >> 1 ] + cpu.fs_idx_lo[ opc[ 3 ] ], cpu.d[ opc[ 2 ] & 1 ], cpu.fl_idx_hi[ opc[ 3 ] ] );
        cpu.cycles++;
    }
    cpu.pc += 4;
    cpu.cycles += 16 + cpu.fl_idx_hi[ opc[ 3 ] ];
}
...
{
    int len = ( opc[ 3 ] == 0xF ) ? 5 : ( ( cpu.fl_idx_hi[ opc[ 3 ] ] == 1 ) ? 17 : cpu.fl_idx_hi[ opc[ 3 ] ] );
...
{
    int len = ( opc[ 3 ] == 0xF ) ? 5 : cpu.fl_idx_hi[ opc[ 3 ] ];
...
{
    if ( !( opc[ 2 ] & 8 ) ) {
        comp_eq( REGuF( opc[ 2 ], opc[ 1 ] ), REGvF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
    } else {
        comp_zero( cpu.reg[ opc[ 2 ] & 3 ] + cpu.fs_idx_lo[ opc[ 1 ] ], cpu.fl_idx_hi[ opc[ 1 ] ] );
    }
    if ( opc[ 2 ] & 4 )
        cpu.carry = !cpu.carry;
    goyes( opc, 3 );
    cpu.cycles += 6 + cpu.fl_idx_hi[ opc[ 1 ] ];
}

static void op9b( byte* opc ) // ?u>v f / u<v f / u>=v f / u<=v f
{
    comp_gt( REGuF( opc[ 2 ], opc[ 1 ] ), REGvF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
    if ( opc[ 2 ] & 8 )
        cpu.carry = !cpu.carry;
    goyes( opc, 3 );
    cpu.cycles += 6 + cpu.fl_idx_hi[ opc[ 1 ] ];
...
static void opAa( byte* opc ) // t=t+v f / t=t-1 f
{
    switch ( opc[ 2 ] & 0xC ) {
        ...
        case 0x4:
        case 0x8:
            alu_add( REGtF( opc[ 2 ], opc[ 1 ] ), REGvF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
        case 0xC:
            alu_dec( REGtF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
    }
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
}
...
static void opAb( byte* opc ) // t=0 f / t=r f / trEX f
{
    switch ( opc[ 2 ] & 0xC ) {
        case 0x0:
            reg_zero( REGtF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
        case 0x4:
        case 0x8:
            reg_cpy( REGtF( opc[ 2 ], opc[ 1 ] ), REGrF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
        case 0xC:
            reg_ex( REGtF( opc[ 2 ], opc[ 1 ] ), REGrF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
    }
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
...
static void opBa( byte* opc ) // t=t-v f / t=t+1 f / t=v-t f
{
    switch ( opc[ 2 ] & 0xC ) {
        case 0x0:
        case 0x8:
            alu_sub( REGtF( opc[ 2 ], opc[ 1 ] ), REGvF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
        case 0x4:
            alu_inc( REGtF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
        case 0xC:
            alu_sub2( REGtF( opc[ 2 ], opc[ 1 ] ), REGvF( opc[ 2 ], opc[ 1 ] ), cpu.fl_idx_hi[ opc[ 1 ] ] );
            break;
    }
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
...
{
    alu_sl( cpu.reg[ opc[ 2 ] & 3 ] + cpu.fs_idx_lo[ opc[ 1 ] ], cpu.fl_idx_hi[ opc[ 1 ] ] );
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
}
...
{
    alu_sr( cpu.reg[ opc[ 2 ] & 3 ] + cpu.fs_idx_lo[ opc[ 1 ] ], cpu.fl_idx_hi[ opc[ 1 ] ] );
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
}
...
{
    alu_neg( cpu.reg[ opc[ 2 ] & 3 ] + cpu.fs_idx_lo[ opc[ 1 ] ], cpu.fl_idx_hi[ opc[ 1 ] ] );
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
}
...
{
    alu_not( cpu.reg[ opc[ 2 ] & 3 ] + cpu.fs_idx_lo[ opc[ 1 ] ], cpu.fl_idx_hi[ opc[ 1 ] ] );
    cpu.pc += 3;
    cpu.cycles += 3 + cpu.fl_idx_hi[ opc[ 1 ] ];
}
